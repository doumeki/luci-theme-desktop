...
- **luci-lua-runtime** — the theme controller is Lua-based. On ucode-only
  ...
  It is a declared package dependency and is installed **automatically** —
  no manual step needed for normal installs. Only if it is missing
  (e.g. a manual/no-deps install), add it yourself:
